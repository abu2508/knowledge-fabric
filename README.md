...
| **Laya** (`laya.py`) | Convai Innovations' Laya ("System 1": input state + typed questions in, structured answer + calibrated probability out, one fast forward pass) isn't callable from this environment. Stands in with a structured-output call to Claude Haiku 4.5 behind the exact same `score(input_state, question) -> LayaAnswer` interface every caller uses - swap the class body for a real Laya client without touching `linking.py` or `retrieval.py`. |
